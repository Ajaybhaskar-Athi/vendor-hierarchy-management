...
const DeployedTasks = () => {
  ...
  const vehicles = useSelector((state) =>
    state.deployAssociates.vehicles.filter((v) => v.assignedTo === loggedInUser.id)
  );

  // Get deploy associates under this Local Vendor
  const deployAssociates = useSelector((state) =>
    state.deployAssociates.deployAssociates.filter((d) => d.parentId === loggedInUser.id)
  );
